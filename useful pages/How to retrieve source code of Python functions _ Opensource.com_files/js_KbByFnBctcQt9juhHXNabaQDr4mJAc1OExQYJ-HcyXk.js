    ...
    attach: function (context) {
      (function() {
        var COOKIE_DOMAIN = window.location.hostname,
          COOKIE_NAME = "redhat_cookie_privacy_banner",
          BANNER_ID = "redhat-cookie-privacy-banner",
          // NOTE: Set in load()
          BANNER;

        function isOldIE(element) {
          return element.attachEvent && typeof element.attachEvent !== "undefined";
        }
        ...
        function close() {
          ...
          document.cookie = COOKIE_NAME + "=; path=/; domain=" + COOKIE_DOMAIN + "; expires=" + expires.toUTCString();
        ...
        function load() {
          // Create BANNER element and append to
          BANNER = document.getElementById(BANNER_ID);

          // Set "close" click event
          var closeElement = document.getElementById(BANNER_ID + "-close");
          if (isOldIE(closeElement)) {
            closeElement.attachEvent("onclick", close);
          } else {
            closeElement.addEventListener("click", close);
          }

          // Set display based on existence of cookie
          var hasCookieRegExp = new RegExp("(?:^|.*;\\s*)" + COOKIE_NAME + "\\s*\\=")
          display(!hasCookieRegExp.test(document.cookie));
        }
